 ...
 */
async function run(platform, resourcesDirectory, options, resizeOptions, errstream) {
    debug('Running %s platform with options: %O', platform, options);
    const resources = [];
    const sources = [];
    const adaptiveResult = await safelyGenerateAdaptiveIconResources(platform, resourcesDirectory, options["adaptive-icon" /* ADAPTIVE_ICON */], resizeOptions, errstream);
    if (adaptiveResult && adaptiveResult.resources.length > 0) {
        ...
        const iconResult = await generateSimpleResources("icon" /* ICON */, platform, resourcesDirectory, options["icon" /* ICON */], resizeOptions, errstream);
        if (iconResult) {
            resources.push(...iconResult.resources);
            sources.push(iconResult.source);
        }
    }
    const splashResult = await generateSimpleResources("splash" /* SPLASH */, platform, resourcesDirectory, options["splash" /* SPLASH */], resizeOptions, errstream);
    if (splashResult) {
 ...
 */
async function safelyGenerateSimpleResources(type, platform, resourcesDirectory, options, resizeOptions, errstream) {
    if (!options) {
        return;
    }
    try {
        return await generateSimpleResources(type, platform, resourcesDirectory, options, resizeOptions, errstream);
    }
    catch (e) {
 ...
 * with `undefined`.
 */
async function generateSimpleResources(type, platform, resourcesDirectory, options, resizeOptions, errstream) {
    if (!options) {
        return;
    }
    ...
    const resources = await Promise.all(config.resources.map(async (resource) => ({
        ...resource,
        ...(await generateImageResource(type, platform, resourcesDirectory, config, source.image, { ...resource, ...resizeOptions }, getResourceTransformFunction(platform, type, options), errstream)),
    })));
    return {
        resources,
        source,
...
function getResourceTransformFunction(platform, type, { transform = (image, pipeline) => pipeline, }) {
    const transforms = [transform];
    if (platform === "ios" /* IOS */ && type === "icon" /* ICON */) {
 ...
 */
async function safelyGenerateAdaptiveIconResources(platform, resourcesDirectory, options, resizeOptions, errstream) {
    if (!options || platform !== "android" /* ANDROID */) {
        return;
    }
    try {
        return await generateAdaptiveIconResources(resourcesDirectory, options, resizeOptions, errstream);
    }
    catch (e) {
 ...
 */
async function generateAdaptiveIconResources(resourcesDirectory, options, resizeOptions, errstream) {
    ...
    const { resources: iconResources = [], source: iconSource, } = (await safelyGenerateSimpleResources("icon" /* ICON */, "android" /* ANDROID */, resourcesDirectory, options.icon, resizeOptions, errstream)) || { source: undefined };
    const { resources: foregroundResources, source: foregroundSource, } = await generateAdaptiveIconResourcesPortion(resourcesDirectory, "foreground" /* FOREGROUND */, options.foreground.sources, options.foreground.transform, resizeOptions, errstream);
    ...
    const backgroundResources = resolvedBackgroundSource.type === "raster" /* RASTER */
        ? await generateAdaptiveIconResourcesPortionFromImageSource(resourcesDirectory, "background" /* BACKGROUND */, resolvedBackgroundSource, options.background.transform, resizeOptions, errstream)
 ...
 */
async function generateAdaptiveIconResourcesPortion(resourcesDirectory, type, sources, transform = (image, pipeline) => pipeline, resizeOptions, errstream) {
    ...
    return {
        resources: await generateAdaptiveIconResourcesPortionFromImageSource(resourcesDirectory, type, source, transform, resizeOptions, errstream),
        source,
    };
}
exports.generateAdaptiveIconResourcesPortion = generateAdaptiveIconResourcesPortion;
async function generateAdaptiveIconResourcesPortionFromImageSource(resourcesDirectory, type, source, transform = (image, pipeline) => pipeline, resizeOptions, errstream) {
    ...
    const resources = await Promise.all(config.resources.map(async (resource) => ({
        ...resource,
        ...(await generateImageResource("adaptive-icon" /* ADAPTIVE_ICON */, "android" /* ANDROID */, resourcesDirectory, config, source.image, { ...resource, src: resource[type], ...resizeOptions }, transform, errstream)),
    })));
    return resources;
}
exports.generateAdaptiveIconResourcesPortionFromImageSource = generateAdaptiveIconResourcesPortionFromImageSource;
async function generateImageResource(type, platform, resourcesDirectory, config, image, schema, transform = (image, pipeline) => pipeline, errstream) {
    ...
    await image_1.generateImage(generatedImage, transform(generatedImage, pipeline.clone()), metadata, errstream);
    return {
        type,
        format,
        width,
